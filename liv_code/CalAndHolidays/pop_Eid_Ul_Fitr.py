...
START_HIJRI_YEAR = 1346       # approximately 1927 CE
END_HIJRI_YEAR = 1526         # approximately 2125 CE
...
def apparent_alt_az(body, jd):
    ...
    # Sidereal time
    sidtime = swe.sidtime(jd)

    # Convert RA to hour angle in degrees.
    hour_angle = sidtime * 15.0 - ra
...
def find_ramadan_ending_new_moon(ramadan_anchor):
    """
    Ramadan is nominally 29 or 30 days.

    Search for the New Moon around 29-30 days after
    1 Ramadan.

    We use the astronomical New Moon nearest
    Ramadan day 29/30.
    """

    day_29 = ramadan_anchor + timedelta(days=28)
    day_30 = ramadan_anchor + timedelta(days=29)

    jd29 = find_new_moon_near(day_29)
    jd30 = find_new_moon_near(day_30)

    # They are actually the same New Moon.
    # Choose the one closer to day 29/30 center.
    target = ramadan_anchor + timedelta(days=29)

    target_jd = datetime_to_jd(target)

    if abs(jd29 - target_jd) < abs(jd30 - target_jd):
        return jd29
...
def process_ramadan_row(row, conn):
    ...
    anchor_date = datetime.strptime(
        row["Ramadan_Anchor_Date"],
        "%Y-%m-%d"
    ).date()

    print(
        f"\nProcessing Hijri {hijri_year} "
        f"(Ramadan begins {anchor_date})"
    )
    ...
    if observation["visibility"] in (
        ...
        rule = (
            ...
            "30 days and Eid assigned to 1 Shawwal."
        )
